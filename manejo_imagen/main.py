...
def divide_image_into_sectors_rgb(image, sector_size=100):
    # Dimensiones de la imagen
...
def save_sectors_color_to_file(sector_matrices, filename, separacion, num_sectores):
    # Abrir el archivo para escribir
    with open(filename, 'w') as file:
        # Recorrer todos los sectores
        contador = 0
        for sector_num, sector in sector_matrices.items():
            ...
            if (contador == num_sectores):
                return 0
            for row in sector:
                # Convertir los valores de cada píxel RGB a hexadecimal y unirlos en una cadena
                row_hex = f'{separacion}'.join(f'{r:02X} {g:02X} {b:02X}' for r, g, b in row)
                # Escribir la fila en el archivo
                file.write(row_hex + '\n')
            contador += 1
            file.write('\n')  # Separador entre sectores
...
def save_combined_sectors_color_to_file(sector_matrices, filename="output_sectors_combined_color.txt"):
    # Abrir el archivo para escribir
    with open(filename, 'w') as file:
        # Crear una imagen de 400x400 a partir de los sectores 100x100
        combined_matrix = np.zeros((400, 400, 3), dtype=np.uint8)
...
output_sector_rgb = os.path.join(repo_base_path, 'memoria', 'sectores_100x100_rgb.txt')
output_completo_rgb = os.path.join(repo_base_path, 'memoria', 'imagen_completa_rgb.txt')


# Cargar la imagen en escala de grises usando la ruta generada
greyscale = cv2.imread(path_imagen, cv2.IMREAD_GRAYSCALE)
rgb = cv2.imread(path_imagen, cv2.IMREAD_COLOR)
...
if greyscale is None:
    ...
    sector_matrices_rgb = divide_image_into_sectors_rgb(rgb, sector_size=100)

    if sector_matrices_greyscale and sector_matrices_rgb:
        ...
        save_sectors_color_to_file(sector_matrices_rgb, output_sector_rgb, ' ', 16)
        save_combined_sectors_color_to_file(sector_matrices_rgb, output_completo_rgb)
